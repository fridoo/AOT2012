...
public final class TemperatureSensor extends AbstractAgentBean implements ResultReceiver {
	...
	public void execute() {
		...
		final IActionDescription heating = thisAgent.searchAction(new Action(Heating.ACTION_GET_HEATING_STATE));
		if (heating != null) {
			this.invoke(heating, new Serializable[] {}, this);
		}
		final IActionDescription window = thisAgent.searchAction(new Action(Window.ACTION_GET_WINDOW_STATE));
		if (window != null) {
